...
class MyPyEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, verbose_env=False):
        ...
        self._is_short = np.int32(0)
        self._is_flat = np.int32(1)
        self._is_long = np.int32(0)
        ...
        self.verbose = verbose_env

    def get_state(self):
        return [self._num_steps, self._entry_pos, self._is_short, self._is_flat, self._is_long, self._mid_price, self._MA_slow, self._MA_fast, self._MA_close]
    ...
    def _step(self, action):
        ...
        if self.verbose:
            print(f"[Episode {self._epi_counter }][{self._num_steps}] Action: {action}              ", end="\r")
        self._num_steps += 1
    ...
    def print_state(self, reward = None, final=False):
        ...
        state_string += f"[pos type: {pos_type}],"
