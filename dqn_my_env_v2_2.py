...
class MyPyEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, verbose_env=False):
        ...
        self._observation_spec = {
            ...
            'pos_price':array_spec.BoundedArraySpec(shape=(2,), dtype=np.int32, minimum=0, maximum=1, name='obs_pos_price'),
    ...
    def init_prices(self):
        self._prices = np.empty(shape=(1000,))
        self._prices[0] = np.random.uniform(low=0.0, high=100.0)
        self._MA_slow = np.zeros_like(self._prices)
        self._MA_fast = np.zeros_like(self._prices)
        self._MA_close = np.zeros_like(self._prices)
        ...
        for time in range(1,1000):
            ...
            # if step == 0:
            #     if self._mid_price > 1:
            #         self._prices[time] = self._prices[time - 1] - 1
            #     else:
            #         self._prices[time] = self._prices[time - 1]
            # if step == 1:
            #     self._prices[time] = self._prices[time - 1] + 1
            
            if time > 200:
                self._MA_slow = np.average(self._prices[time-200:time])
    ...
    def get_state(self):
        return {
            'price': [self._mid_price, self._MA_slow, self._MA_fast, self._MA_close],
